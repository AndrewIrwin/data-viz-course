...
library(sf) # simple features = map objects (points, lines, areas)
library(ggrepel)
```
...
ggplot(data = canada_sf) +
  ...
  theme(
    panel.background = element_rect(fill = "#0022FF"),
...
map1 <- canada |> ggplot() + 
  ...
  geom_sf(data = city_coords, color = "red") +
  theme_void() +
  theme(
    panel.background = element_rect(fill = "aliceblue"),
    panel.grid = element_blank()
  ) +
    coord_sf(crs = st_crs(3978))  # Ask LLM for help
map1 
```
...
map1 + geom_label_repel(
    ...
    fill = "white",
    alpha = 0.8,
    nudge_y = 100000 # Shift labels up (in meters if using CRS 3347)
  )
```
