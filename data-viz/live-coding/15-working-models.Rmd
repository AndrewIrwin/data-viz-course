...
Fit a gam for both year and month.

```{r}
m_gam <- gam(co2 ~ s(year) + s(X2), data= co2)
library(gratia)
draw(m_gam)
m_gam <- gam(co2 ~ poly(year,2) + s(X2), data= co2)
draw(m_gam)
summary(m_gam)
```
