...
Order from smallest to largest, top to bottom. Watch out for NAs. `median` is the default function.

```{r fig.align="center", fig.asp=0.75, message=FALSE, warning=FALSE, dev='svglite', out.width="50%"}
penguins |> na.omit() |>
  ggplot(aes(x = body_mass_g, 
             y = fct_reorder(species, body_mass_g, mean,
                             .desc=TRUE))) + 
  stat_summary() + my_theme + labs(y="Species")
```
...
## Grouping factors together

```{r}
mpg |> ggplot() +
  geom_point(aes(cty, displ, color = manufacturer)) + 
  theme_bw()
```
...
```{r}
mpg |> 
  mutate(manufact = fct_lump(manufacturer, n = 3)) |>
  ggplot() +
  geom_point(aes(cty, displ, color = manufact)) + 
  theme_bw()
```
...
```{r}
mpg |> 
  mutate(manufact = fct_lump_prop(manufacturer, prop = 0.10)) |>
  ggplot() +
  geom_point(aes(cty, displ, color = manufact)) + 
  theme_bw()
```
...
```{r}
mpg |> 
  mutate(manufact = fct_lump_min(manufacturer, min = 15)) |>
  ggplot() +
  geom_point(aes(cty, displ, color = manufact)) + 
  theme_bw()
```
...
```{r}
make_date(2026, 1:12, 1) # first day of each month
# make a sequence of the last day of each month
start_date <- as.Date("2026-01-01")
end_date <- as.Date("2026-12-01")
first_days_seq <- seq(ceiling_date(start_date, "month"), 
                      ceiling_date(end_date, "month"), by = "month")
first_days_seq - days(1)
```
...
tibble(date = c("2021/03/11 10:05", "2021/03/12 15:12", 
       ...
       dt = ymd_hm(date),
       value = 1:4) |>
  mutate(elapsed = dt - min(dt)) |> 
  arrange(elapsed) |> kable()
```
