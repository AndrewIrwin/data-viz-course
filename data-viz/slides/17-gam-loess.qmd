...
* LOESS: locally estimated scatterplot smoothing (quadratic, distance weighted)
...
library(mgcv)
m1 <- gam( mean_temp ~ s(year_day, bs="cc"), data = hfx)
summary(m1)
```
...
```{r fig.height=5, fig.align="center"}
library(gratia)
hfx |> add_fitted(m1) |> select(year_day, mean_temp, .fitted) |>
...
confint(m1, "s(year_day)", level = 0.95) |>
  ...
  geom_line(aes(y=.estimate), size=2, color="red") + 
...
* Model statistics available with `glance` for GAM (but not LOESS)
