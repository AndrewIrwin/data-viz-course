...
library(readxl)
library(lubridate)
cal <- read_excel(here::here(".", "schedule.xlsx"))
```
...
cal |> select(-Day) |>
  ...
  mutate(Date = strftime(Date + 12*3600, format = "%a %e %b", 
         ...
         Exercises = case_when(!is.na(evaluation) ~ my_paste(evaluation, evaluation_link),
                               TRUE ~ my_paste(exercise, exercise_link, "003-exercises.qmd")),
